...
def get_emails_from_page(url):
    ...
    try:
        ...
        # Extract email addresses from elements with class "field__item" or "span"
        emails = [email.text.strip() for email in soup.find_all(class_=['field__item', 'span'], string=re.compile(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b'))]
        return emails
...
def main():
    main_page_url = 'https://www.sternekessler.com/professionals/list/O'
    ...
    try:
        ...
        for link in links:
            ...
            # Get emails from elements with class "field__item" or "span" on the linked page
